...
As part of my work at Obtego, we needed to ensure the most accurate possible retrieval from a medical knowledge graph, even in scenarios where the question was abstract. Because of this, my research turned up an approach to knowledge graph retrival that used latent space to also enhance exploration of that graph before answering.
...
Inspired by Harvard's work on [KGARevion](https://openreview.net/pdf?id=tnB94WQGrn), I extended Coconut with a graph neural network (GNN) encoding layer that aligns the embeddings of the graph with those of the model.
